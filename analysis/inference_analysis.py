...
SCENARIOS = ["flat", "step", "crossrtt"]   # display order: flat > step > crossrtt
...
def plot_cdfs(df):
    """Single figure with three horizontal panels: flat > step > crossrtt.
    Each panel has one CDF curve per model, pooling all steps across all
    seeds and episodes. Shaded band = min/max envelope across per-seed CDFs.
    """
    fig, axes = plt.subplots(
        1, len(SCENARIOS),
        figsize=(6 * len(SCENARIOS), 5),
        sharey=True,   # all panels share the same y (CDF) axis
    )
    ...
    for ax, scenario in zip(axes, SCENARIOS):
        for model_type in MODEL_TYPES:
            ...
            # Grand CDF — pool all steps across all seeds and episodes
            all_times = df[
            ...
            # Per-seed CDF envelope — interpolated onto common x grid
            x_grid    = np.linspace(sorted_t.min(),
                                    np.percentile(sorted_t, 99.5), 500)
            seed_cdfs = []
            for seed in SEEDS:
                seed_t = df[
                    (df["model_type"] == model_type) &
                ...
                seed_cdfs.append(
                    np.interp(x_grid, s_sorted, s_cdf, right=1.0)
                )
        ...
        ax.text(ax.get_xlim()[1], 0.951, " p95", va="bottom",
                fontsize=7, color="grey")
        ax.text(ax.get_xlim()[1], 0.991, " p99", va="bottom",
                fontsize=7, color="grey")

        ax.set_title(SCENARIO_LABELS[scenario], fontsize=11, fontweight="bold")
        ax.set_xlabel("Inference Time (ms)", fontsize=10)
        ax.set_ylabel("CDF" if ax == axes[0] else "", fontsize=10)
        ax.set_ylim(0, 1.02)
        ax.set_xlim(left=0)
    ...
    fig.legend(handles, labels,
               loc="upper center", ncol=len(MODEL_TYPES),
               fontsize=9, framealpha=0.9,
               bbox_to_anchor=(0.5, 1.02))

    fig.suptitle("Inference Time CDF", fontsize=12, fontweight="bold", y=1.07)
    fig.tight_layout()

    for ext in ("png", "pdf"):
        path = os.path.join(OUT_DIR, f"cdf_all.{ext}")
        fig.savefig(path, dpi=DPI, bbox_inches="tight")
    plt.close(fig)
    print("Saved: cdf_all.png/pdf")
